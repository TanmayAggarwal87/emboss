...
import { TextProcessingError } from "../../src/lib/text-processing/errors.ts";
...
test("failed OCR language initialization reports failure and its owned worker can be stopped", { timeout: 15_000 }, async () => {
  const worker = new Worker(join(process.cwd(), "src/lib/text-processing/ocr-worker.cjs"), {
    workerData: { png: new Uint8Array(), languagePath: join(process.cwd(), "tests/text-processing/missing-language-fixture") },
  });
  try {
