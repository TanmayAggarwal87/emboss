...
`[2026-09-26]` - Source preview URLs depended on the upload worker's process-local memory. A separate Vercel request/instance can return 410 immediately even before TTL expiry. Upload/retry responses now deliver the exact MuPDF PNG crops as response-only data URLs; the review UI consumes these in browser memory without a second server request. A shared 2 MiB character budget bounds inline crops per response; oversized crops report a preview-only error. No PDF/raster persistence, database migration, package installation, or additional Gemini call is required. Cache expiry and separate-instance regression tests cover the failure; legacy URL handling remains supported. Reset/reload clears browser crops, so older responses cannot recover lost source images.

`[2026-09-18]` - Phase 6 Preview UI implements side-by-side verification of original PDF source crops against client-side Three.js tactile meshes. Source crops are generated during upload via MuPDF rasterization and held in an in-memory bounded LRU cache (50 MB cap, 15-minute TTL); they are never sent back to Gemini and never stored in Supabase or an external bucket. Because caching is process-local, multi-instance serverless deployments or process restarts return HTTP 410 (Gone) upon expiry or cache miss. The preview viewer uses `createPreviewMesh(geometry)`, preserving the exact same underlying THREE.Group handle across preview and future Phase 8 STL export (`AGENTS.md` §7). `Tactile3DViewer` uses `next/dynamic` with `ssr: false`, handles WebGL context loss explicitly via `webglcontextlost` listeners, and configures `preserveDrawingBuffer: true` on `THREE.WebGLRenderer` so headless CDP screen captures and software renderers (SwiftShader) preserve drawn buffer contents across compositor passes. Camera navigation provides accessible buttons for keyboard reviewers with full aria labels. Approval, editing and export remain inactive and reserved for Phases 7 and 8.
...
`[2026-09-17]` - Phase 4 verification passed 13 new tests plus 40 regression tests, lint, strict TypeScript and production build. A generated two-page chart PDF supplied known labels/values: live Call B matched the bar chart immediately; the first line request failed with a service error. After a cooldown spent on build/docs, one line-only request matched exactly and passed Supabase read-back. Total live requests: 3. Both diagnostic jobs and their regions were removed. `npm run verify:diagram-extraction -- --live --chart line --database` checks only the line fixture; omit `--live` for zero model traffic. Existing Phase 2/3 diagnostics remain quota-free through scoped fixture classification/injected diagram perception.
...
`[2026-09-17]` - Text processing verification made zero Gemini calls: 13 Phase 1 tests, 15 Phase 2 tests, normal/scanned fixture comparisons, and the supplied two-page text PDF passed. Supabase read-back matched both synthetic fixtures and their diagnostic jobs were deleted. `npm run verify:text-processing -- --database` repeats that check without model use; see `docs/text-processing-verification.md`. OCR is English-only, successes require review, and partially scanned pages with an existing text layer do not silently use OCR for empty regions.
