...
import { EXPECTED_CHARTS } from "../tests/phase4/fixtures.ts";
import { buildGeometryMesh, disposeGeometryMesh } from "../src/lib/phase5/mesh.ts";
import { generateGeometry } from "../src/lib/phase5/generate.ts";
import { BRAILLE, DEFAULT_PROFILE } from "../src/lib/phase5/profile.ts";
import { validateGeometry } from "../src/lib/phase5/validate.ts";
import type { GeometryResult, GeometryState } from "../src/lib/phase5/types.ts";
...
try {
  ...
  if (values.artifacts) {
    const folder = await mkdtemp(join(tmpdir(), "emboss-phase5-"));
    const { createDiagramFixture } = await import("../tests/phase4/fixtures.ts");
  ...
  if (values.database) {
    ...
    const rows = states.map((geometry, index) => ({ region_id: `phase5-chart-${index + 1}`, type: "diagram" as const,
