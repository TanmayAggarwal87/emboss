...
import { GeminiDiagramExtractor } from "../src/lib/diagram-extraction/gemini.ts";
import { parseDiagramResponse } from "../src/lib/diagram-extraction/schema.ts";
import { getSupabaseAdmin } from "../src/lib/supabase/admin.ts";
import { CHART_BOX, EXPECTED_CHARTS, createDiagramFixture } from "../tests/diagram-extraction/fixtures.ts";
...
try {
  const folder = values.artifacts ? await mkdtemp(join(tmpdir(), "emboss-diagram-extraction-")) : undefined;
  if (folder) {
