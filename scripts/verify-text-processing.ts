...
import { createTextFixture, TEXT_BOX, REFERENCE_TEXT, REFERENCE_GRADE_1, REFERENCE_GRADE_2 } from "../tests/text-processing/fixtures.ts";
...
async function verifyFixture(scanned: boolean) {
  ...
  try {
    ...
    form.set("file", new File([Uint8Array.from(createTextFixture({ scanned }))], "text-processing-fixture.pdf", { type: "application/pdf" }));
...
try {
  ...
  console.log("Text processing verification passed. Gemini calls: 0.");
} catch (error) {
  console.error(error instanceof Error ? error.message : "Text processing verification failed.");
  process.exitCode = 1;
} finally {
