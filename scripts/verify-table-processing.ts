...
import type { JobRepository, PersistedRegion } from "../src/lib/document-processing/types.ts";
...
import { LocalTextRecognizer } from "../src/lib/text-processing/ocr.ts";
...
import { createTableFixture, TABLE_BOX, TABLE_HEADERS, TABLE_ROWS } from "../tests/table-processing/fixtures.ts";
...
globalThis.fetch = (input, init) => {
  ...
  if (!values.database || !allowed || url.origin !== new URL(allowed).origin) {
    throw new Error("Table processing verification forbids network requests except configured Supabase with --database.");
  }
...
try {
  ...
  form.set("file", new File([Uint8Array.from(bytes)], "table-processing-fixture.pdf", { type: "application/pdf" }));
  ...
  if (values.artifacts) {
    const folder = await mkdtemp(join(tmpdir(), "emboss-table-processing-"));
  ...
  console.error(error instanceof Error ? error.message : "Table processing verification failed.");
  process.exitCode = 1;
} finally {
