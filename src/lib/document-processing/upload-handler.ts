...
import type { TextProcessor } from "../text-processing/types.ts";
import type { TableProcessor } from "../table-processing/types.ts";
import type { DiagramProcessor } from "../diagram-extraction/types.ts";
import type { GeometryProcessor } from "../tactile-geometry/types.ts";
...
export type UploadDependencies = {
  ...
  sourcePreviews?: SourcePreviewStore;
};
...
async function processPage(dependencies: UploadDependencies, session: RetrySession,
  ...
  try {
    ...
    // Capture from the still-open original PDF. A preview failure must not undo
    // saved extraction/geometry or cause another model call.
    if (dependencies.sourcePreviews) {
      for (const region of regions) {
        try {
          region.source_preview = dependencies.sourcePreviews.put(session.jobId!, region.id,
            document.rasterizeRegion(pageNumber - 1, region.bounding_box));
        } catch {
          region.source_preview = { error: "This source crop could not be rendered. Refer to the original PDF to compare this region." };
        }
      }
    }
